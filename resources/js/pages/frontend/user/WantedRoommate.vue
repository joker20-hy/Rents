<template>
  <div contain-box class="mt-4">
    <form v-if="room" @submit.prevent="create()" class="bg-white p-3" style="box-shadow: 0px 0px 15px #e0e0e0;border-top: 5px solid #3490dc;">
      <div class="text-center">
        <h1 style="font-size: x-large" class="mb-3">Tìm người ở ghép</h1>
      </div>
      <div class="form-group text-center text-muted text-small">
        {{ `Phòng chỉ có thể có tối đa ${room.limit_renter} người thuê` }}
      </div>
      <label>Số người mong muốn <span class="text-danger">*</span></label>
      <div class="form-group">
        <input type="number" class="form-control" v-model="number" placeholder="Số người ở cùng phòng mong muốn" required>
      </div>
      <!--  -->
      <label>Thông tin liên hệ</label>
      <div class="form-group">
        <textarea class="form-control" v-model="contact" placeholder="vd: số điện thoại 0123456789"></textarea>
      </div>
      <!--  -->
      <label>Thông tin thêm</label>
      <div class="form-group">
        <textarea class="form-control" v-model="content" placeholder="vd: Phòng hiện đang rất đủ đồ, chỉ cần chuyển đến ở"></textarea>
      </div>
...
export default {
  ...
  computed: {
    ...
    left_slot() {
      return this.room.limit_renter-this.room.renter_count
    }
  },
  created() {
  ...
  methods: {
    create() {
      if (this.number > this.left_slot) {
        this.error = {status: true, message: `Phòng không thể có quá ${this.room.limit_renter} người thuê`}
        setTimeout(() => {
      ...
      ajax().post(`/api/renter/wanted-roommate`, {
        number: this.number,
      ...
      .then(res => {
        ...
        $eventHub.$emit('success-alert', {
          title: 'Thành công',
          message: 'Tin được đăng thành công',
          timeout: 3000
        })
      ...
      .catch(err => {
        ...
        $eventHub.$emit('error-alert', {
          ...
          message: 'Hiện chưa thể đăng tin, hãy thử lại',
          timeout: 3000
        })
      })
    },
